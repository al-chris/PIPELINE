...
def upload_to_cloudinary_task(self: Task[Any, Any], file_bytes: bytes, filename: str, task_id: str) -> str:
    # file_bytes: the binary content of the uploaded file
    # task_id: the same ID used as the annotation task
    async def _upload():
...
@celery.task(bind=True)
def db_commit_file_annotation(self: Task[Any, Any], file_url: str, task_id: str):
    async def _commit():
